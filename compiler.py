...
def generate_assembly(tempquad, name):
        ...
        for q in quad[sq-1:] :
                ...
                assembly_file.write( f'L_{q[0]}:\n' )

                if q[1] == 'jump' :

                        assembly_file.write( f'\t j L_{q[4]}\n' )
                        ...
                        assembly_file.write( f'\t beq $t1,$t2,L_{q[4]}\n' )

                elif q[1] == '<>' :

                        loadvr(q[2], 1)
                        loadvr(q[3], 2)
                        assembly_file.write( f'\t bne $t1,$t2,L_{q[4]}\n' )
                        ...
                        assembly_file.write( f'\t bgt $t1,$t2,L_{q[4]}\n' )
                        ...
                        assembly_file.write( f'\t blt $t1,$t2,L_{q[4]}\n' )
                        ...
                        assembly_file.write( f'\t bge $t1,$t2,L_{q[4]}\n' )
                        ...
                        assembly_file.write( f'\t ble $t1,$t2,L_{q[4]}\n' )
                        ...
                        assembly_file.write( '\t addi $a0,$t1,0\n' )
                        ...
                        if name == mainname :
                                ...
                                assembly_file.write( '\t addi $fp,$sp,' + str(mainFL) + '\n' )
                        ...
                        if len(parameters) == 0 :
                                ...
                                assembly_file.write( f'\t add $fp,$sp,{fl}\n' )
                        ...
                        if q[3] == 'CV' :
                                ...
                                assembly_file.write(f'\t sw $t0, -{distance}($fp)\n')
                                ...
                                if qname.typeof == 'var' and qnest == level :
                                        ...
                                        assembly_file.write( f'\t addi $t0, $sp,-{qname.offset}' )
                                        assembly_file.write( f'\t sw $t0, -{distance}($fp)\n' )
                                ...
                                if qname.typeof == 'par' and qnest == level and qname.parMode == 'REF' :
                                        ...
                                        assembly_file.write( f'\t lw $t0,-{qname.offfset}($sp)\n' )
                                        assembly_file.write( f'\t sw $t0, -{distance}($fp)\n' )
                        ...
                        if name == mainname :
                                ...
                                assembly_file.write( f'\t addi $fp,$sp,{mainFL}\n' )
                        ...
                        if qnest == level :
                                ...
                                assembly_file.write( f'\t addi $sp,$sp,{fl}\n' )
                                assembly_file.write( f'\t jal L_{qname.SQ}\n' )
                                assembly_file.write( f'\t addi $sp,$sp,-{fl}\n' )
                        ...
                        assembly_file.write( f'\t addi $sp,$sp,{mainFL}\n' )
...
def generate_intermediate_representation(tempquad):

        for x in tempquad :

                print (x)
                intermediate_representation_file.write( f'{x}' )
                intermediate_representation_file.write('\n')
        
        
def generate_c(tempquad):
        ...
        for x in tempquad :
                ...
                if x[1] == 'jump' :
                        ...
                        c_file.write( f'  goto L_{x[4]}; //({x[1]}, {x[2]}, {x[3]}, {x[4]})\n' )
                if x[1] == '+' or x[1] == '-' or x[1] == '*' or x[1] == '/' :

                        c_file.write( f'  L_{x[0]}: {x[4]}={x[2]}{x[1]}{x[3]}; //({x[1]}, {x[2]}, {x[3]}, {x[4]})\n' )

                if x[1] == '=' :

                        c_file.write( f'  L_{x[0]}: if({x[2]}=={x[3]}) goto L_{x[4]}; //({x[1]}, {x[2]}, {x[3]}, {x[4]})\n')

                if x[1] == ':=' :

                        c_file.write( f'  L_{x[0]}: {x[4]}={x[2]}; //({x[1]}, {x[2]}, {x[3]}, {x[4]})\n' )

                if x[1] == '<' or x[1] == '>' or x[1] == '<=' or x[1] == '>=' :

                        c_file.write( f'  L_{x[0]}: if({x[2]}{x[1]}{x[3]}) goto L_{x[4]}; //({x[1]}, {x[2]}, {x[3]}, {x[4]})\n' )
                ...
                if x[1] == 'halt' :
                        ...
                        c_file.write( f'  L_{x[0]}: {{}}\n' )

                if x[1] == 'out' :

                        c_file.write( f'  L_{x[0]}: printf("%d/n",{x[2]}); //({x[1]}, {x[2]}, {x[3]}, {x[4]})\n' )

                if x[1] == 'inp' :

                        c_file.write( f'  L_{x[0]}: scanf("%d",{x[2]}); //({x[1]}, {x[2]}, {x[3]}, {x[4]})\n' )
...
def gnvlcode(name):
        ...
        if not(fname.typeof == 'func') :
                ...
                assembly_file.write( f'\t addi $t0,$t0,-{fname.offset}\n' )
...
def loadvr(v, r):
        ...
        if v.isdigit() :
                ...
                assembly_file.write( f'\t li $t{r},{v}\n' )
        ...
        if fname.typeof == 'var' :
                ...
                if len(scopes) == 0 :
                        ...
                        assembly_file.write( f'\t lw $t{r},-{fname.offset}($s0)\n ' )

                if nest == scopes[ len(scopes) - 1 ].nestingLevel :

                        assembly_file.write( f'\t lw $t{r},-{fname.offset}($sp)\n' )

                if nest < scopes[ len(scopes) - 1 ].nestingLevel :

                        gnvlcode(v)
                        assembly_file.write( '\t lw $t' + str(r) + ',0($t0)\n' )
                ...
                if fname.parMode == 'REF' and nest == scopes[ len(scopes) - 1 ].nestingLevel :
                        ...
                        assembly_file.write( f'\t lw $t0,-{fname.offset}($sp)\n' )
                        assembly_file.write( '\t lw $t' + str(r) + ',0($t0)\n' )
...
def storerv(r, v):
        ...
        if fname.typeof == 'var' :
                ...
                if len(scopes) == 0 :
                        ...
                        assembly_file.write( f'\t sw $t{r},-{fname.offset}($s0)\n' )

                if nest == scopes[ len(scopes) - 1 ].nestingLevel :

                        assembly_file.write( f'\t sw $t{r},-{fname.offset}($sp)\n' )

                if nest < scopes[ len(scopes) - 1 ].nestingLevel :

                        gnvlcode(fname.name)
                        assembly_file.write( '\t sw $t' + str(r) + ',($t0)\n' )
...
def tokenizer():
    ...
    while position < len(source_code) :
        ...
        if source_code[position] == '+' and state == 0 :
        ...
        elif (source_code[position] == '\t' or source_code[position].isspace()) and state == 0 :
        ...
        elif source_code[position] == '-' and state == 0 :
            ...
            state = 6
            position = position + 1
            if source_code[position] == '/' and state == 6 :

                token = '//'
                tokenstr = 'commenttk'
                position = position + 1

            elif source_code[position] == '*' and state == 6 :
        ...
        elif not(source_code[position] == '\n') and state == 6 :

                position = position + 1

        elif not(source_code[position] == '*') and state == 7 :

                if token == '/*' and position == len(source_code) -1 :
        ...
        elif source_code[position] == '*' and state == 7 :

            position = position + 1
            if source_code[position] == '/' and state == 7 :
            ...
            position = position + 1
            state = 3
            if source_code[position] == '>' and state == 3 :
            ...
            elif source_code[position] == '=' and state == 3 :
            ...
            position = position + 1
            state = 4
            if source_code[position] == '=' and state == 4 :
            ...
            position = position + 1
            state = 5
            if source_code[position] == '=' and state == 5 :
            ...
            elif not(source_code[position] == '=') and state == 5 :
        ...
        elif source_code[position].isalpha() and state == 0 :

            state = 1
            token = source_code[position]
            position = position + 1

        elif (source_code[position].isalpha() or source_code[position].isdigit()) and state == 1 :

                token = token + source_code[position]
                position = position + 1

        elif (not(source_code[position].isalpha() or source_code[position].isdigit())) and state == 1 :
        ...
        elif source_code[position].isdigit() and state == 0 :

            state = 2
            token = source_code[position]
            position = position + 1

        elif source_code[position].isdigit() and state == 2 :

            token = token + source_code[position]
            position = position + 1

        elif source_code[position].isalpha() and state == 2 :

            print('Invalid number')
            exit(1)

        elif not(source_code[position].isdigit() and state == 2) :
...
def main():
    global source_code, tokenboard, assembly_file, intermediate_representation_file, c_file, state, lines, position
    ...
    with open(os.path.join(output_dir, pure_name + '.int'), 'w') as intermediate_representation_file, \
         open(os.path.join(output_dir, pure_name + '.c'), 'w') as c_file, \
         open(os.path.join(output_dir, pure_name + '.asm'), 'w+') as assembly_file:

        state = 0
        lines = 0
        position = 0
        tokenboard = tokenizer()
        assembly_file.write('\t j L_  \n')
        program()
        assembly_file.seek(0)
        assembly_file.write(f'\t j L_{mainSQ}\n')
        generate_intermediate_representation(quad)
        generate_c(quad)
